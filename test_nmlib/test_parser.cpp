...
TEST(ParserTest, OneModuleType){
    ...
    EXPECT_EQ(1, typeManager.numUserTypes());
...
TEST(ParserTest, multipleModuleTypes){
    nm::Parser parser;
    ...
    nm::TypeManager &typeManager = *(*maybeTypeManager);
    EXPECT_EQ(2, typeManager.numUserTypes());

    nm::ModuleType* terrainModuleType = typeManager.getUserType("terrain");
    ASSERT_NE(nullptr, terrainModuleType) << "Couldn't find ModuleType terrain";

    nm::ModuleType* mountainTerrainModuleType = typeManager.getUserType("mountainTerrain");
    ASSERT_NE(nullptr, mountainTerrainModuleType) << "Couldn't find ModuleType mountainTerrain";
}
